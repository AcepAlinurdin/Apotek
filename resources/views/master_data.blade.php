<x-app-layout>
                    ...
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <div class="flex space-x-2">
                            ...
                            <a href="{{ route('suppliers.index') }}"
                                class="inline-flex items-center px-4 py-2 bg-indigo-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-600">
                                Kelola Supplier
                            </a>
                        </div>
                    </div>
                    {{-- PERUBAHAN: Tidak ada perubahan di form, hanya di div stok di bawah --}}
                    <form id="formObat">
                        ...
                        <div class="grid md:grid-cols-4 grid-cols-2 gap-4">
                            ...
                            <div class="md:col-span-2">
                                <label for="nama_obat" class="block text-sm font-medium text-gray-700">Nama
                                    Obat</label>
                                <input id="nama_obat" name="nama_obat" type="text" placeholder="Nama Lengkap Obat"
                                    class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            ...
                            {{-- PERUBAHAN 1: Tambahkan ID pada div pembungkus stok dan satuan --}}
                            <div id="stok-container" class="md:col-span-1 col-span-2 grid grid-cols-3 gap-2">
                            ...
                            <div class="md:col-span-1">
                                <label for="harga_satuan" class="block text-sm font-medium text-gray-700">Harga
                                    Satuan</label>
                                <input id="harga_satuan" name="harga_satuan" type="number" placeholder="0"
                                    class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                <div class="text-red-500 text-xs mt-1 error-text error-harga_satuan"></div>
                            </div>

                            <div class="md:col-span-2">
                                <label for="harga_box" class="block text-sm font-medium text-gray-700">Harga per
                                    Box</label>
                                <input id="harga_box" name="harga_box" type="number" placeholder="0"
                                    class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                        ...
                        <div class="flex space-x-4 mt-6">
                            <button type="submit"
                                class="w-full bg-blue-500 text-white p-2 rounded-md hover:bg-blue-600 font-semibold transition-colors">Simpan</button>
                            <button type="button" id="btn-clear"
                                class="w-1/3 bg-gray-300 text-gray-700 p-2 rounded-md hover:bg-gray-400 font-semibold transition-colors">Batal</button>
                        </div>
                    </form>
                </div>
                <div class="mb-4">
                    <form action="{{ route('obat.master.index') }}" method="GET"
                        ...
                        <input type="text" name="search"
                            ...
                            placeholder="Cari berdasarkan Nama Obat..." value="{{ request('search') }}">
                        <button type="submit"
                            class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700">Cari</button>
                        <a href="{{ route('obat.master.index') }}"
                            class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">Reset</a>
                    </form>
                </div>
                        ...
                        <tbody id="tabelObat" class="text-gray-700 text-sm ">
                            ...
                            <tr id="row-{{ $obat->id }}" class="border-b border-gray-200 hover:bg-gray-100">
                                <td class="py-3 px-6 text-left">
                                    {{ $obat->tanggal ? \Carbon\Carbon::parse($obat->tanggal)->format('d/m/Y') : 'N/A' }}
                                </td>
                                ...
                                <td class="py-3 px-6 text-right">Rp
                                    {{ number_format($obat->harga_box, 0, ',', '.') }}</td>
                                <td class="py-3 px-6 text-center">
                                    <button
                                        class="edit-btn bg-yellow-500 text-white py-1 px-3 rounded hover:bg-yellow-600 text-xs"
                                        data-id="{{ $obat->id }}">Edit</button>
                                    
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="9" class="text-center p-4">Belum ada data obat atau data tidak
                                    ditemukan.</td>
                            </tr>
                            @endforelse
                        </tbody>
    ...
    <script>
        $(document).ready(function () {
            ...
            // PERUBAHAN 2: Lewatkan informasi peran dari Blade ke JavaScript
            const isKepalaApotek = {{ auth()->check() && auth()->user()->hasRole('kepala apotek') ? 'true' : 'false' }};
            ...
            function resetForm() {
                ...
                $('#obat_id').val('');
                $('.error-text').text('');
                $('#formObat button[type="submit"]').text('Simpan').removeClass(
                    'bg-green-500 hover:bg-green-600').addClass('bg-blue-500 hover:bg-blue-600');
                
                // PERUBAHAN 3: Pastikan kolom stok selalu terlihat saat form di-reset (untuk tambah data baru)
                $('#stok-container').show();
            }

            // Fungsi updateTableRow tidak perlu diubah, tapi saya sertakan agar lengkap
            function updateTableRow(obat) {
                let formattedHargaSatuan = formatRupiah(obat.harga_satuan);
                let formattedHargaBox = obat.harga_box ? formatRupiah(obat.harga_box) : 'Rp 0';
                let supplierName = obat.supplier ? obat.supplier.nama_supplier : 'N/A';
                let formattedDate = new Date(obat.tanggal).toLocaleDateString('id-ID', {
                    day: '2-digit',
                    month: '2-digit',
                    year: 'numeric'
                });
                ...
                if (existingRow.length) {
                    existingRow.html(rowContent);
                } else {
                    // Hapus pesan 'data kosong' jika ada sebelum menambahkan baris baru
                    if ($('#tabelObat').find('td[colspan="9"]').length > 0) {
                        $('#tabelObat').empty();
                    }
                    $('#tabelObat').append(
                        `<tr id="row-${obat.id}" class="border-b border-gray-200 hover:bg-gray-100">${rowContent}</tr>`
                    );
                }
            }
            ...
            $('#tabelObat').on('click', '.edit-btn', function () {
                // PERUBAHAN 4: Tambahkan logika untuk menampilkan/menyembunyikan stok
                if (isKepalaApotek) {
                    $('#stok-container').show();
                } else {
                    $('#stok-container').hide();
                ...
                // Mengambil tanggal dari format dd/mm/yyyy dan mengubahnya ke yyyy-mm-dd untuk input date
                let dateParts = row.find('td:eq(0)').text().trim().split('/');
                let formattedDate = `${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`;

                $('#obat_id').val(id);
                $('#tanggal').val(formattedDate);
                $('#nama_obat').val(row.find('td:eq(1)').text().trim());
                $('#kategori').val(row.find('td:eq(2)').text().trim());
                $('#supplier').val(row.find('td:eq(3)').text().trim());
                $('#stok').val(row.find('td:eq(4)').text().trim());
                $('#satuan').val(row.find('td:eq(5)').text().trim());
                $('#harga_satuan').val(row.find('td:eq(6)').text().replace(/[^0-9]/g, ''));
                $('#harga_box').val(row.find('td:eq(7)').text().replace(/[^0-g]/g, ''));
                
                $('#formObat button[type="submit"]').text('Update').removeClass('bg-blue-500')
            ...
            // Logika delete (jika diperlukan kembali) tetap tidak diubah
        });
    </script>
</x-app-layout>